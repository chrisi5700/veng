...
std::expected<Buffer, vk::Result> upload_buffer(vma::Allocator allocator, std::span<const std::byte> bytes,
												vk::BufferUsageFlags usage)
{
	auto buffer = Buffer::create(allocator, bytes.size(), usage, vma::MemoryUsage::eAuto,
...
std::expected<bool, graph::ExecError> MeshNode::record(gpu::GpuExecContext& ctx)
{
	...
	{
		auto vertex = upload_buffer(ctx.allocator(), m_vertex_bytes, vk::BufferUsageFlagBits::eVertexBuffer);
		if (!vertex.has_value())
		{
		...
		{
			auto index = upload_buffer(ctx.allocator(), m_index_bytes, vk::BufferUsageFlagBits::eIndexBuffer);
			if (!index.has_value())
			{
				return std::unexpected(graph::ExecError::NODE_FAILED);
			}
