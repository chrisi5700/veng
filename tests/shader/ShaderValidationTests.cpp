...
// Context is factory-constructed now (Context::create -> std::expected); build one
// and require success so the validation tests below read cleanly.
veng::Context make_context()
{
	auto result = veng::Context::create("veng::Shader Validation Test");
	REQUIRE(result.has_value());
	return std::move(result.value());
}
